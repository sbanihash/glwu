...
# operational version
      ...
      cat $oname >> glwu.t${cyc}z.awipsbull
      rm -f formbul.out $oname
    done

  if [ $SENDCOM = "YES" ] ; then
    #XXW cp  awipsbull.$job $COMOUTwmo/awipsbull.$job
    cp  glwu.t${cyc}z.awipsbull $COMOUTwmo/.
    if [ $SENDDBN = "YES" ] ; then
       #XXW make_ntc_bull.pl  WMOBH NONE KWBJ NONE $DATA/awipsbull.$job $COMOUTwmo/awipsbull.$job
       make_ntc_bull.pl  WMOBH NONE KWBJ NONE $DATA/glwu.t${cyc}z.awipsbull $COMOUTwmo/glwu.t${cyc}z.awipsbull
    fi
  fi
