...
# 2.f Buoy location file    
  ...
  if [ -f $FIXglwu/wave_$buoy.buoys ]  
  then        
    cp $FIXglwu/wave_$buoy.buoys buoy.loc        
  fi
    
  if [ -f buoy.loc ]      
  then        
    echo "   buoy.loc copied ($FIXglwu/wave_$buoy.buoys)."   
  else        
...
# 2.g Create model input file from template       
  ...
  if [ -f $FIXglwu/multiwavefcst.${runID}_lc.tmpl ]    
  then        
    cp $FIXglwu/multiwavefcst.${runID}_lc.tmpl multiwavefcst.inp.tmpl   
  fi
...
# 4.a Send files to com if requested  
  ...
  then        
    ...
    echo "`hostname -s`" > $COMOUT/where_${runID_lc}_ran.$cycle  
    
    if [ -f log.mww3 ] ; then         
      echo "   Copying log.mww3      to $COMOUT/${runID_lc}.$cycle.log" 
      cp               log.mww3         $COMOUT/${runID_lc}.$cycle.log  
    fi        
