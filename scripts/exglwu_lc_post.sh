...
# Updated for Lake Champlain                                   Sep, 2022      #
###############################################################################
# --------------------------------------------------------------------------- #
# 0.  Preparations
...
# 0.c Defining model grids
  ...
  export pntgrd="glwu_lc"
  export grids='glwu_lc'
  export isgrids='grlr_500m_lc' # Compute 500m as source grid for other interpolations
  export itgrids='grlc_2p5km_lc ' # Target grids for distributions 
  export ggrids='grlc_2p5km_lc grlc_2p5km_lc_sr' # grids that require grib output
  export addggrids='grlr_500m_lc' # additional grids that require grib output
  export ngrids='glwu_lc' # Grids with netcdf output

# 0.c.1 Define a temporary directory for storing ascii point output files
#       and flush it

  export STA_DIR=$DATA/lc_station_ascii_files
  if [ -d $STA_DIR ]
  then 
    rm -rf ${STA_DIR}
  fi
...
# 1.d Output locations file
  ...
  do
    rm -f buoy.loc
    if [ -f $FIXglwu/wave_$grdID.buoys ]
    then
      cp $FIXglwu/wave_$grdID.buoys buoy.loc.temp
    ...
    then
      set +x
      echo "   buoy.loc copied and processed ($FIXglwu/wave_$grids.buoys)."
      [[ "$LOUD" = YES ]] && set -x
    else
      set +x
      echo ' '
...
# Assigned NCEP number for model -- MODNR
  ...
  then
    ...
    do
      ...
      then
        case $grdID in
          grlr_500m_lc) GRIDNR=255  ; MODNR=131  ; dtgrib=3600. ; ngrib=150 ; GTMPLN=0 ;;
          grlc_2p5km_lc) GRIDNR=255  ; MODNR=131  ; dtgrib=3600. ; ngrib=150 ; GTMPLN=30 ;;
        ...
        case $grdID in
          ...
          grlc_2p5km_lc) GRIDNR=255  ; MODNR=131  ; dtgrib=3600. ; ngrib=49 ; GTMPLN=30 ;;
          grlc_2p5km_lc_sr) GRIDNR=255  ; MODNR=133  ; dtgrib=3600. ; ngrib=49 ; GTMPLN=30 ;;
        esac
      fi
      echo "RUNID: $runID "
...
# 11.  Ending output
  ...
  then
     ...
     touch $COMOUT/${runID_lc}.$cycle.postdone
  fi
