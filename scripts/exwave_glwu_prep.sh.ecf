...
# 1.a Model definition file
  ...
  do
    ...
    then
      ...
      [[ "$LOUD" = YES ]] && set -x
      if [ -f $FIXwave/wave_$grdID.inp ] || [ -f $FIXwave/mesh.$grdID ]
      then
        cp $FIXwave/wave_$grdID.inp $grdID.inp
        cp $FIXwave/mesh.$grdID mesh.$grdID
      fi
...
# 1.b Wind preprocessor template file
  ...
  if [ -f $FIXwave/multiwaveprnc.${wndTAG}.tmpl ]
  then
    cp $FIXwave/multiwaveprnc.${wndTAG}.tmpl .
  fi

  if [ -f multiwaveprnc.${wndTAG}.tmpl ]
  then
    echo "   multiwaveprnc."${wndTAG}".tmpl copied ($FIXwave/multiwaveprnc."${wndTAG}".tmpl)."
  else
...
#
  ...
  do
    ...
    file=$FIXwave/T_OEBA88_C_KNWC.${ftype}
...
# 2.b Ice preprocessing 
  ...
  do
  echo "$USHwave/waveice_glw.sh $ymdh > ice_$ymdh.err 2>&1"     >> cmdfile
    tinc_ice=1
...
# 2.c ndfd winds
  ...
  do
    if [ "$ymdh" -le "$ymdh_end" ]
    then
      echo "$USHwave/wavendfd_glwu_inc.sh $ymdh 1 $hdr > ndfd_$ymdh.err 2>&1"     >> cmdfile
...
# Here using igrids (the regular grid for ice grlr)
  ...
  do
    ...
    $EXECcode/multiwaveprep > multiwaveprep.out
    err=$?
