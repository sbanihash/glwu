...
!     and from the file multiwavegrads_fields.inp ( NDSI, output requests ).
...
!
      ...
      OPEN (NDSI,FILE=FNMPRE(:JLEN)//'multiwavegrads_fields.inp',STATUS='OLD',       &
            ERR=800,IOSTAT=IERR)
...
!
      ...
      END DO
      IF ( FLREQ(9,5) ) THEN
        WRITE (NDSO,946) IDOUT(9,5),'*** NOT YET CODED INTO WAVEGXFL ***'
        FLREQ(9,5) = .FALSE.
      END IF
...
!
 1000 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     ERROR IN OPENING INPUT FILE'/                    &
               '     IOSTAT =',I5/)
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     IOSTAT =',I5/)
!
 1010 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     SMALLEST OUTPUT INCREMENT IS 60 SEC.'/)
!
 1011 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     ERROR IN OPENING OUTPUT FILE ww3.grads'/         &
               '     IOSTAT =',I5/)
!
 1012 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     ERROR IN OPENING OUTPUT FILE ww3.ctl'/           &
               '     IOSTAT =',I5/)
!
 1020 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
               '     FIELD INCREMENT > 1HR BUT NOT MULTIPLE',F10.0/)
!
 1021 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGXFL : '/               &
