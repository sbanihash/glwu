...
! (again note that "k" is equivalent to isea, but includes *all* points)
            ...
            IF (L_STOP) THEN
                ...
                CHECK_DEALLOC_STATUS ( ISTAT )
              END IF
            IF (L_STOP) CYCLE ! cycle out of LOWRANK_GRID loop
            GSRC   = -999 ! unset grid
...
!        dst grid.
 ...
 !If L_STOP is set, we are done
  IF (L_STOP) THEN
    WRITE (MDSE,*)'Stopping after writing remap files'
    CALL EXTCDE(0) ! not a true error, so code is zero
  END IF
!
