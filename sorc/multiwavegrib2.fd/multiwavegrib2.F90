...
!/                  (J-Henrique Alves)
!/    01-Dec-2016 : Adding lambert conformal grid       ( version 5.16 )
!/                  (J.H. Alves)
!/
...
!     and from the file multiwavegrib2.inp ( NDSI, output requests ).
...
!
      OPEN (NDSI,FILE='multiwavegrib2.inp',STATUS='OLD',ERR=800,IOSTAT=IERR)
...
!
      ...
      IF ( GDTN .NE. 30 ) THEN
        WRITE(NDSE,*)'PROGRAM WAVEGRIB2: CURVILINEAR GRID SUPPORT '// &
        'FOR GRIB OUTPUT IS NOT YET IMPLEMENTED. NOW STOPPING'
        CALL EXTCDE ( 1 )
      ENDIF
      END IF
!
! Coded up to now only for Lamber conformal grids (GDTN=30)
! For regular grids use GDTN=0
!
      IF ( GDTN .EQ. 30 ) THEN
        CALL NEXTLN ( COMSTR , NDSI , NDSE )
...
!
 1000 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGRIB2 : '/               &
               '     ERROR IN OPENING INPUT FILE'/                    &
               '     IOSTAT =',I5/)
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGRIB2 : '/               &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGRIB2 : '/               &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     IOSTAT =',I5/)
!
 1005 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEGRIB2 : '/               &
               '     OUTPUT REQUESTED FOR FIELDS THAT SHARE KPDS(5)'/ &
               '     FIRST FIELD  : ',A/                              &
               '     SECOND FIELD : ',A/)
!
 1010 FORMAT (/' *** WAVEWATCH-III ERROR IN WAVEGRIB2 : '/          &
