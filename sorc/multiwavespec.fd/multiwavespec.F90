...
!     and from the file multiwavespec.inp ( NDSI, output requests ).
...
!
      J      = LEN_TRIM(FNMPRE)
      OPEN (NDSI,FILE=FNMPRE(:J)//'multiwavespec.inp',STATUS='OLD',        &
            ERR=800,IOSTAT=IERR)
...
!
 1000 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ERROR IN OPENING INPUT FILE'/                    &
               '     IOSTAT =',I5/)
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     IOSTAT =',I5/)
!
 1003 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ERROR IN OPENING TABLE FILE'/                    &
               '     IOSTAT =',I5/)
!
 1004 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ERROR IN OPENING IDL FILE'/                      &
               '     IOSTAT =',I5/)
!
 1005 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/          &
               '     ERROR IN OPENING BUOY LOG FILE'/            &
               '     IOSTAT =',I5/)
!
 1007 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/              &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     LAST POINT INDEX IS NOT -1'/                    &
               '     OR TOO MANY POINT INDEXES DEFINED'/)
!
 1010 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ILLEGAL TYPE, ITYPE =',I4/)
!
 1011 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
               '     ILLEGAL TYPE, OTYPE =',I4/)
!
 1012 FORMAT (/' *** WAVEWATCH III ERROR IN WAVESPEC : '/               &
...
!          user should set the 5th T/F value in multiwavespec.inp for
...
!       A: The Sbot source function in multiwavespec was originally written
...
!             multiwavespec (instead of just ww3_shel as is the case now).
!
!  8. Structure :
!
!     See source code.
...
!
                ...
                IF ( FLSRCE(5) ) THEN
                 ...
                 BT8MSG='multiwavespec: ITYPE=3 with BT8 or BT9: Sbot out'//&
