...
!     and from the file multiwavefldn.inp ( NDSI, output requests ).
...
!
      J      = LEN_TRIM(FNMPRE)
      OPEN (NDSI,FILE=FNMPRE(:J)//'multiwavefldn.inp',STATUS='OLD',       &
            ERR=800,IOSTAT=IERR)
...
!
 1000 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEFLDS : '/               &
               '     ERROR IN OPENING INPUT FILE'/                    &
               '     IOSTAT =',I5/)
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEFLDS : '/               &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEFLDS : '/               &
               '     ERROR IN READING FROM INPUT FILE'/               &
               '     IOSTAT =',I5/)
!
 1010 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEFLDS : '/               &
               '     ILLEGAL TYPE, NCTYPE =',I4/)
 1012 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEFLDS : '/               &
               '     NCTYPE = 4 but multiwavefldn is compiled without NC4'/)
!/
...
!
            ELSE IF ( IFI .EQ. 2 .AND. IFJ .EQ. 13 ) THEN
              ...
              NFIELD=1
              varnm(1)='hmaxe'
              varnl(1)='expected maximum wave height (nonlinear,2nd order)'
              varns(1)='expected maximum wave height (nonlinear,2nd order)'
              varng(1)='expected maximum wave height (nonlinear,2nd order)'
              varnc(1)=''
              vmin = 0
              vmax = 32000
!
            ELSE IF ( IFI .EQ. 2 .AND. IFJ .EQ. 14 ) THEN
              ...
              NFIELD=1
              varnm(1)='hcmaxe'
              varnl(1)='expected maximum wave height from crest (nonlinear,2nd order)'
              varns(1)='expected maximum wave height from crest (nonlinear,2nd order)'
              varng(1)='expected maximum wave height from crest (nonlinear,2nd order)'
              varnc(1)=''
              vmin = 0
              vmax = 32000
!
            ELSE IF ( IFI .EQ. 2 .AND. IFJ .EQ. 15 ) THEN
              ...
              NFIELD=1
              varnm(1)='hmaxd'
              varnl(1)='STD of maximum wave height (nonlinear,2nd order)'
              varns(1)='STD of maximum wave height (nonlinear,2nd order)'
              varng(1)='STD of maximum wave height (nonlinear,2nd order)'
              varnc(1)=''
              vmin = 0
              vmax = 32000
!
            ELSE IF ( IFI .EQ. 2 .AND. IFJ .EQ. 16 ) THEN
              ...
              NFIELD=1
              varnm(1)='hcmaxd'
              varnl(1)='STD of maximum wave height from crest (nonlinear,2nd order)'
              varns(1)='STD of maximum wave height from crest (nonlinear,2nd order)'
              varng(1)='STD of maximum wave height from crest (nonlinear,2nd order)'
              varnc(1)=''
              vmin = 0
              vmax = 32000
!
