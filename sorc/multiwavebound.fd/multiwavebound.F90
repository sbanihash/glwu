...
! 3. Read input file
!
      OPEN(NDSI,FILE='multiwavebound.inp',status='old')
...
!
      DO ILOOP = 1, 2
        OPEN (NDSS,FILE='multiwavebound.scratch',FORM='FORMATTED',          &
              status='UNKNOWN')
        IF ( ILOOP .EQ. 1 ) THEN
          NDSI2 = NDSI
        ELSE
...
!
 1001 FORMAT (/' *** WAVEWATCH-III ERROR IN WAVEBOUND : '/              &
               '     PREMATURE END OF INPUT FILE'/)
!
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEBOUND: '/               &
               '     ERROR IN READING ',A,' FROM INPUT FILE'/         &
               '     IOSTAT =',I5/)
!
 1006 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEBOUND: '/               &
               '     ILLEGAL XFR, XFR =',F12.6/)
!
 1007 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEBOUND: '/               &
               '     ILLEGAL NTH, NTH =',I3,' DIFFERS FROM NTHI =',I3/)
!
 1008 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEBOUND: '/               &
               '     ILLEGAL NK, NK =',I3,' DIFFERS FROM NKI =',I3/   &
               '     IT WILL BE MANAGED SOON BY SPCONV')
!
 1010 FORMAT (/' *** WAVEWATCH III ERROR IN WAVEBOUND: '/               &
               '     ILLEGAL TYPE, ITYPE =',I4/)
!/
!/ End of WAVEBOUND ---------------------------------------------------- /
!/
      END PROGRAM WAVEBOUND
!/ ------------------------------------------------------------------- /
