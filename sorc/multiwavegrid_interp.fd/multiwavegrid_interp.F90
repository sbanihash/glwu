...
!   their (and target grid) file extns are read from 'multiwavegrid_interp.inp'.
...
!
        J      = LEN_TRIM(FNMPRE)
        OPEN(NDSI,FILE=FNMPRE(:J)//'multiwavegrid_interp.inp',STATUS='OLD', ERR=2000,      &
             IOSTAT=IERR)
        WRITE (NDSO,900)
!
...
!
        REWIND (NDSI)
        READ (NDSI,*,END=2001,ERR=2002) COMSTR
        IF ( COMSTR .EQ. ' ' ) COMSTR = '$'
...
!
          ...
          IF ( ICLOSE .EQ. ICLOSE_TRPL ) THEN
             WRITE(NDSE,*)'PROGRAM WAVEGRID_INTERP HAS NOT BEEN '//  &
...
!
        ...
        CALL NEXTLN ( COMSTR, NDSI, NDSE )
        READ (NDSI,*,END=2001,ERR=2002) EXTRA
...
! Now dumps the coefficients to file ...
        ...
        DO ISEA = 1, NSEA
          ...
          DO IG = 1,COUNTG
            ...
            COUNTF = GR_INTS(ISEA)%IND_WTS(COUNTG)%NP
            WRITE(994) COUNTF
            DO I = 1,COUNTF
...
!/
!/ End of WAVEGRID_INTERP -----------------------------------------------------/
!/
        END PROGRAM WAVEGRID_INTERP
